...
<p align="center">
  <a href="https://github.com/noderelay/UplinkIRC/releases/latest/download/Uplink-2026.8.0-x86_64.AppImage">
  ...
  &nbsp;
  <a href="https://github.com/noderelay/UplinkIRC/releases/latest/download/Uplink-v2026.8.0-linux-x86_64.tar.gz">
  ...
  &nbsp;
  <a href="https://github.com/noderelay/UplinkIRC/releases/latest/download/Uplink-v2026.8.0-windows-x64.zip">
  ...
  &nbsp;
  <a href="https://github.com/noderelay/UplinkIRC/releases/latest/download/Uplink-v2026.8.0-macos-arm64.dmg">
...
| **Channel panes** | Right-click any `#channel` in the sidebar → **Open in Pane**. Up to 4 panes total. Each pane has its own chat history, nick list, topic bar (with toggle), search, and input bar. Auto-layout: 2 = side by side, 3 = primary left + two stacked right, 4 = 2×2 grid. The split axis follows the window shape by default (wide = columns, tall = rows), controlled by **Preferences → Interface → Split Panes Automatically**; untick it to keep the panes exactly as they are. Grab any pane's header (the main view included) to rearrange: the pane lifts out and follows your cursor, its old slot fills with a theme color, and the drop target highlights with a frame. Drop on a pane's middle to swap the two, or on its left/right/top/bottom edge to place the dragged pane on that side, which sets the axis to match. Channels open in a pane never accumulate unread badges: you're already watching them. Click inside a pane and the sidebar loads into *that* pane, leaving the layout alone; click the main view first and sidebar clicks go back to loading there. |
...
| **Profiles, avatars & channel icons** | IRCv3 `draft/metadata-2`/`-3`. Display name, avatar and status text show in nick tooltips; ops set a channel icon with `/chanavatar` and it appears in every member's sidebar. Both are optional: **Preferences → Chat Window → Show Avatars** keeps metadata but skips the image fetch (an avatar URL points at someone else's host, which sees your IP), and **Manage Servers → Use metadata** skips the capability entirely for a given server. |
