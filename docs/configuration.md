...
| `pane_split_axis` | string | `"auto"` | Which way 2+ [detachable channel panes](howto.html#channel-panes) split. `"auto"` picks from the shape of the pane area: a wide area splits into columns (side by side), a tall one into rows (stacked). `"columns"` and `"rows"` force it, and are what an edge-drop or unticking **Preferences → Interface → Split Panes Automatically** writes. The axis is re-evaluated when panes are opened, closed, or rearranged, not while you resize the window. Replaces the old `pane_stack_rows` bool, which still migrates if it was set to `true`. |
