...
pane_split_axis = "auto"          # "auto" (default) follows the window shape, or "columns" / "rows"
panel_cards = true                # side panels float as rounded cards on the backdrop; false = classic flat look
menu_style = "menubar"    # "menubar" File/Edit/... bar (joins the KDE global menu), "hidden" shortcuts only
persistence = "default"   # stay on the server while disconnected: "default" (server decides), "on", "off"
app_icon = "flat-black"
...
| `persistence` | string | `"default"` | Whether the server should keep your nick online while Uplink is disconnected, like a bouncer (IRCv3 [`draft/persistence`](ircv3.html), Ergo's always-on). `"on"` and `"off"` are sent to every server that supports the capability, on connect and live when changed from **Preferences → Interface → Stay Online**; `"default"` sends nothing and leaves the server's own setting alone. Requires a registered account (SASL or NickServ). See [Server-side persistence](howto.html#persistence). |
