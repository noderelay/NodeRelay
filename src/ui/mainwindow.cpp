...
#endif
                const int ret = QMessageBox::question(this, "Update Available",
                    QString("Uplink %1 is available (you are on v" UPLINK_VERSION ").\n\n%2?")
...
void MainWindow::downloadAndApplyUpdate(const QString &url, const QString &assetName)
{
    const QString tempPath = QDir::tempPath() + "/" + assetName;

    auto *f = new QFile(tempPath, this);
    if (!f->open(QIODevice::WriteOnly)) {
        QMessageBox::warning(this, "Update Failed", "Could not create temporary file.");
        f->deleteLater();
        return;
    }

    auto *progress = new QProgressDialog("Downloading " + assetName + "...", "Cancel", 0, 100, this);
...
void MainWindow::applyUpdate(const QString &tempPath, const QString &assetName)
{
#if defined(Q_OS_LINUX)
    Q_UNUSED(assetName);
    ...
    QMessageBox::information(this, "Update Ready", "Uplink has been updated. It will now restart.");
