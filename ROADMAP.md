...
winget: abandoned 2026-07-31. Both submission PRs sat in Microsoft's manual-review queue since 2026-07-17; when a moderator finally ran validation, Defender false-flagged the (hash-verified, untampered) release zip as `Trojan:Win32/Sprisky.U!cl` — a known cloud-heuristic misfire on unsigned low-prevalence binaries. Not worth the dispute process; both PRs closed. Windows users get the portable zip from the releases page. AUR (×3) and the Homebrew tap are current and need no action.
