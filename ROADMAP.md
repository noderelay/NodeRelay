...
**Scripting** — user script bindings (Preferences → Scripts) with sandboxed QProcess execution; four bundled scripts (/music, /weather, /uptime, /roll) working on Linux, Windows, macOS. More bundled scripts planned above. Since 2026-07-16: **plugins** — long-running external processes speaking JSON lines over stdin/stdout (src/plugins/pluginprotocol + src/ui/plugincontroller); events (message/join/part/quit/nick/kick/topic/connected/disconnected) in, actions (say/me/print/command) out; crash restart with 3-strike cutoff, per-plugin flood guard, /plugins status command, Preferences → Plugins page, three commented example plugins auto-installed disabled, full beginner guide in docs/plugins.md.
