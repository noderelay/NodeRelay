...
- [x] Full-history search v2: all buffers — "All buffers" toggle in the Ctrl+Shift+F window scans every log under `logs/` on the worker thread; results grouped per buffer (bold header with match count, newest 200 matches each); double-click a result to jump to that buffer (`SessionModel::resolveLogBuffer()` maps sanitized log path components back to the live buffer) (2026-07-11)
- [x] Theming overhaul — per-section theme backgrounds honored ([sidebar]/[nicklist] were flattened to buffer bg); full-height side panels drawn as rounded-top cards; user-list bottom gap fixed (layout stretch + Wayland styled-background drop — new ChromePanel paints chrome fills in paintEvent; widget masks removed as unsafe at fractional display scale) (2026-07-11)
- [x] Panel Cards toggle — Preferences → Interface → Panel Cards (`panel_cards`, default on): switch between the two-tone rounded-card side panels and the classic flat look, live (2026-07-11)
...
- [x] Pane splitter + macOS user-list frame fixes (2026-07-13, PR #61) — pane dividers were pinned by un-elided header labels (channel name / topic-setter enforce full text width as a layout minimum; the column holding the primary panel couldn't shrink on small screens). New `ElidedLabel` (elide on squeeze, full text on hover) for the primary and pane headers; word-wrapped topic text gets a 1px minimum so long URLs can't pin the splitter; pane splitters clamp at pane minimums instead of snap-collapsing columns to zero. On macOS the user-list card was missing its right/bottom frame gaps: stale QSS backgrounds on `#nickPanel`/`#nickPanelHeader` painted the full widget rect under the ChromePanel fill (KDE/Wayland drops QSS backgrounds on plain QWidgets, so Linux never showed it) — rules removed, ChromePanel owns those fills
